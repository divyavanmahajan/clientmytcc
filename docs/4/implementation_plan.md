...
2. **Implement `set_credentials`**:
   ...
   - Also store the email in `~/.config/evohome_rs/config.toml` (or similar) so we know which user to look up, OR just use a fixed key like "default_user" in keyring if we only support one user, OR prompt for email and look up password.
